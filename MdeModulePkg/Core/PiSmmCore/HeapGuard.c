...
{
  if (mOnGuarding || (AllocateType == AllocateAddress) || !MPS_VALID) {
    return FALSE;
  }

  if ((HeapGuardType == HeapGuardTypePool) && gMps->Mm.HeapGuard.PoolGuardEnabled) {
    return gMps->Mm.PoolGuard.EnabledForType[MemoryType];
  }

  if ((HeapGuardType == HeapGuardTypePage) && gMps->Mm.HeapGuard.PageGuardEnabled) {
    return gMps->Mm.PageGuard.EnabledForType[MemoryType];
  }
...
{
  if ((Memory == 0) || !MPS_VALID || (!gMps->Mm.HeapGuard.GuardAlignedToTail)) {
    //
