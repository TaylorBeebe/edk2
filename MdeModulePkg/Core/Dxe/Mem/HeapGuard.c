  ...
  )
{
  ...
  //
  Attributes = 0;
  if (MPS_VALID && gMps->Dxe.ExecutionProtection.EnabledForType[EfiConventionalMemory]) {
...
{
  if ((AllocateType == AllocateAddress) || !MPS_VALID) {
    return FALSE;
  }
  ...
  if ((HeapGuardType == HeapGuardTypePool) && gMps->Dxe.HeapGuard.PoolGuardEnabled) {
    return gMps->Dxe.PoolGuard.EnabledForType[TestMemoryType];
  } else if ((HeapGuardType == HeapGuardTypePage) && gMps->Dxe.HeapGuard.PageGuardEnabled) {
    return gMps->Dxe.PageGuard.EnabledForType[TestMemoryType];
  }
  ...
  )
{
  ...
  //
  if (MPS_VALID && gMps->Dxe.HeapGuard.GuardAlignedToTail) {
...
{
  if ((Memory == 0) || !MPS_VALID || (!gMps->Dxe.HeapGuard.GuardAlignedToTail)) {
    //
...
{
  if (MPS_VALID && gMps->Dxe.HeapGuard.FreedMemoryGuardEnabled) {
  ...
  )
{
  ...
  if ((gMps->Dxe.HeapGuard.PageGuardEnabled ||
