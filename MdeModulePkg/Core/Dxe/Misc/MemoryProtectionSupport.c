...
PROTECTED_IMAGES_PRIVATE_DATA  mProtectedImageRecordsPrivate = {
  PROTECTED_IMAGE_PRIVATE_DATA_SIGNATURE,
  0,
  0,
  INITIALIZE_LIST_HEAD_VARIABLE (mProtectedImageRecordsPrivate.ImageRecordList)
};
  ...
  )
{
  ...
  *MemoryMapSize = 0;
  ...
  AdditionalRecordCount = ((2 * mProtectedImageRecordsPrivate.CodeSegmentCountMax + 3) * mProtectedImageRecordsPrivate.ImageRecordCount) +
  ...
  *MemoryMap = ExpandedMemoryMap;
  ...
  DEBUG_CODE (
    ...
    DumpImageRecords (&mProtectedImageRecordsPrivate.ImageRecordList);
    );
  ...
  if (mProtectedImageRecordsPrivate.ImageRecordCount == 0) {
    ...
    MergedImageList = &mProtectedImageRecordsPrivate.ImageRecordList;
  }
  ...
  )
{
  ...
  switch (ProtectionPolicy) {
    case DO_NOT_PROTECT:
      // Clear the attributes of the image in case protection attributes were applied to the buffer
      if ((gCpu != NULL) && (((UINTN)LoadedImage->ImageBase % EFI_PAGE_SIZE) == 0) && ((LoadedImage->ImageSize % EFI_PAGE_SIZE) == 0)) {
        SetUefiImageMemoryAttributes (
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      // Record the image record in the list so we can undo protections later
      Status = OrderedInsertUint64Comparison (
                 &mProtectedImageRecordsPrivate.ImageRecordList,
                 &ImageRecord->Link,
                 OFFSET_OF (IMAGE_PROPERTIES_RECORD, ImageBase) - OFFSET_OF (IMAGE_PROPERTIES_RECORD, Link),
                 OFFSET_OF (IMAGE_PROPERTIES_RECORD, Signature) - OFFSET_OF (IMAGE_PROPERTIES_RECORD, Link),
                 IMAGE_PROPERTIES_RECORD_SIGNATURE
      ...
      mProtectedImageRecordsPrivate.ImageRecordCount++;
      ...
      if (mProtectedImageRecordsPrivate.CodeSegmentCountMax < ImageRecord->CodeSegmentCount) {
        mProtectedImageRecordsPrivate.CodeSegmentCountMax = ImageRecord->CodeSegmentCount;
      }
  ...
  )
{
  ...
  for (ImageRecordLink = mProtectedImageRecordsPrivate.ImageRecordList.ForwardLink;
       ImageRecordLink != &mProtectedImageRecordsPrivate.ImageRecordList;
       ImageRecordLink = ImageRecordLink->ForwardLink)
  {
    ImageRecord = CR (
                    ImageRecordLink,
  ...
  )
{
  ...
  CoreFreePages ((EFI_PHYSICAL_ADDRESS)(UINTN)NULL, 1);
  ...
  )
{
  ...
  if (gDxeMps.NullPointerDetectionPolicy.Fields.UefiNullDetection) {
    ...
    if (gDxeMps.NullPointerDetectionPolicy.Fields.UefiNullDetection) {
      // If both DisableEndOfDxe and DisableReadyToBoot are enabled, disable at EndOfDxe
