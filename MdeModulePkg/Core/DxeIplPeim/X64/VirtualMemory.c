...
{
  if (MPS_VALID (&mMps) && mMps.Dxe.NullPointerDetection.Enabled && (Address == (UINTN)NULL)) {
    return TRUE;
  }

  if (MPS_VALID (&mMps) && mMps.Dxe.CpuStackGuardEnabled) {
    if ((StackBase >= Address) && (StackBase < (Address + Size))) {
      return TRUE;
    }
  }

  if (MPS_VALID (&mMps) && mMps.Dxe.StackExecutionProtectionEnabled) {
  ...
  )
{
  ...
  *PageEntry2M = (UINT64)(UINTN)PageTableEntry | AddressEncMask | IA32_PG_P | IA32_PG_RW;
  ...
  for (IndexOfPageTableEntries = 0; IndexOfPageTableEntries < 512; IndexOfPageTableEntries++, PageTableEntry++, PhysicalAddress4K += SIZE_4KB) {
    ...
    if ((MPS_VALID (&mMps) && mMps.Dxe.NullPointerDetection.Enabled && (PhysicalAddress4K == (UINTN)NULL)) ||
        (MPS_VALID (&mMps) && mMps.Dxe.CpuStackGuardEnabled && (PhysicalAddress4K == StackBase)))
    {
