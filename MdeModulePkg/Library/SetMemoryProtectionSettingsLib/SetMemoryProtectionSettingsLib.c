...
/**
  ...
  @retval EFI_ALREADY_STARTED   The memory protection settings have already been allocated.
**/
STATIC
EFI_STATUS
  ...
  )
{
  ...
  ASSERT (sizeof (MEMORY_PROTECTION_SETTINGS_PRIVATE) < EFI_PAGE_SIZE);

  // Allocate a page for the settings so they can be page protected
  Ptr = AllocatePages (1);
  if (Ptr == NULL) {
    ASSERT (Ptr != NULL);
  ...
  ZeroMem (Ptr, EFI_PAGES_TO_SIZE (1));
  Address = (EFI_PHYSICAL_ADDRESS)Ptr;

  // Publish the HOB entry containing the memory protection settings address
  ...
  // If the HOB could not be published, free the memory and return an error
  if (Ptr == NULL) {
    FreePages (Ptr, 1);
    return EFI_OUT_OF_RESOURCES;
  }

  *Mpsp                          = (MEMORY_PROTECTION_SETTINGS_PRIVATE *)(UINTN)Address;
  ...
  )
{
  ...
  if ((Ptr == NULL)) {
    ...
    Mpsp = (MEMORY_PROTECTION_SETTINGS_PRIVATE *)(UINTN)*((EFI_PHYSICAL_ADDRESS *)GET_GUID_HOB_DATA (Ptr));
  }
  ...
  )
{
  ...
  Status = MemoryPpi->SetPermissions (
                        MemoryPpi,
                        (UINTN)mMpsp,
                        EFI_SIZE_TO_PAGES (sizeof (MEMORY_PROTECTION_SETTINGS)),
  ...
  )
{
  ...
  return MPS_VALID ((&(mMpsp->Mps)))                                                                                                              &&
         (mMpsp->Mps.Dxe.CpuStackGuardEnabled                                                                                                     ||
  ...
  )
{
  ...
  return MPS_VALID ((&(mMpsp->Mps)))                                                                                                            &&
         (mMpsp->Mps.Mm.NullPointerDetection.Enabled                                                                                            ||
